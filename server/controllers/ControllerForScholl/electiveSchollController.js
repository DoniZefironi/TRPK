...
class ElectiveSchollController {
    ...
    async createElective(req, res, next) {
        try {
            ...
            const newElective = await ElectiveScholl.create({
                id_user,
                name,
                topic_elective,
            });
    ...
    async getElectives(req, res, next) {
        try {
            const electives = await ElectiveScholl.findAll();
    ...
    async getElectiveById(req, res, next) {
        try {
            const { id } = req.params;
            const elective = await ElectiveScholl.findByPk(id);
    ...
    async deleteElective(req, res, next) {
        try {
            const { id } = req.params;
            const deleted = await ElectiveScholl.destroy({ where: { id: id } });
...
module.exports = new ElectiveSchollController();