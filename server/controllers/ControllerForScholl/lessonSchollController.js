const { LessonScholl } = require('../../models/models'); // Убедитесь, что путь к модели указан правильно
const ApiError = require('../../error/ApiError');

class LessonSchollController {
    // Создать урок
    async createLesson(req, res, next) {
        try {
            ...
            const newLesson = await LessonScholl.create({
                name,
                id_class,
                topic_lesson,
                id_materials,
    ...
    async getLessons(req, res, next) {
        try {
            const lessons = await LessonScholl.findAll();
...
async updateLesson(req, res, next) {
    try {
        ...
        // Найти урок по ID
        const lesson = await LessonScholl.findByPk(id);
    ...
    async deleteLesson(req, res, next) {
        try {
            const { id } = req.params;
            const deleted = await LessonScholl.destroy({ where: { id_lesson: id } });
...
module.exports = new LessonSchollController();