...
class ClassSchollController {
    ...
    async createClass(req, res, next) {
        try {
            ...
            const newClass = await ClassScholl.create({
                id_user,
                name_class,
                list_user,
            });
    ...
    async getAllClasses(req, res, next) {
        try {
            const classes = await ClassScholl.findAll();
            res.json(classes);
        } catch (error) {
            console.error(error);
    ...
    async getClassById(req, res, next) {
        try {
            const { id } = req.params;
            const classItem = await ClassScholl.findByPk(id);
...
module.exports = new ClassSchollController();