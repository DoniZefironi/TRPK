const { OlympiadScholl } = require('../../models/models'); // Проверьте правильность пути
const ApiError = require('../error/ApiError');

class OlympiadSchollController {
    // Создать олимпиаду
    async createOlympiad(req, res, next) {
        try {
            ...
            const newOlympiad = await OlympiadScholl.create({
                id_user,
                name,
                topic_olympiads,
                date,
    ...
    async getOlympiads(req, res, next) {
        try {
            const olympiads = await OlympiadScholl.findAll();
    ...
    async getOlympiadById(req, res, next) {
        try {
            const { id } = req.params;
            const olympiad = await OlympiadScholl.findByPk(id);
    ...
    async deleteOlympiad(req, res, next) {
        try {
            const { id } = req.params;
            const deleted = await OlympiadScholl.destroy({ where: { id_olympiads: id } });
...
module.exports = new OlympiadSchollController();