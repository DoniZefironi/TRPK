...
class ResultSchollController {
    ...
    async createResult(req, res, next) {
        try {
            ...
            const newResult = await OlympiadResultsScholl.create({
                id_olympiads,
                team_name,
                score,
                position,
    ...
    async getResults(req, res, next) {
        try {
            const results = await OlympiadResultsScholl.findAll();
    ...
    async getResultById(req, res, next) {
        try {
            const { id } = req.params;
            const result = await OlympiadResultsScholl.findByPk(id);
    ...
    async deleteResult(req, res, next) {
        try {
            const { id } = req.params;
            const deleted = await OlympiadResultsScholl.destroy({ where: { id_result: id } });
...
module.exports = new ResultSchollController();