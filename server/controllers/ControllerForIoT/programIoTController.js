const { InternshipProgramIoT } = require('../../models/models'); // Проверьте путь к модели
const ApiError = require('../../error/ApiError');

class ProgramIoTController {
    ...
    async createProgram(req, res, next) {
        try {
            ...
            const newProgram = await InternshipProgramIoT.create({
                program_name,
                program_description,
                program_duration,
                start_date_application,
    ...
    async getPrograms(req, res, next) {
        try {
            const programs = await InternshipProgramIoT.findAll();
    ...
    async getProgramById(req, res, next) {
        try {
            const { id } = req.params;
            const program = await InternshipProgramIoT.findByPk(id);
    ...
    async deleteProgram(req, res, next) {
        try {
            ...
            const deleted = await InternshipProgramIoT.destroy({ where: { id } });
...
module.exports = new ProgramIoTController();