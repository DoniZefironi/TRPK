const { ForumSectionElectric } = require('../../models/models'); // Убедитесь, что путь к модели указан верно
const ApiError = require('../../error/ApiError');

class ForumSectionElectricController {
    ...
    async createForumSection(req, res, next) {
        try {
            ...
            const newForumSection = await ForumSectionElectric.create({
                subsections,
                topic_subsections,
                moderators,
                id_user,
    ...
    async getForumSections(req, res, next) {
        try {
            const forumSections = await ForumSectionElectric.findAll();
    ...
    async getForumSectionById(req, res, next) {
        try {
            const { id } = req.params;
            const forumSection = await ForumSectionElectric.findByPk(id);
    ...
    async deleteForumSection(req, res, next) {
        try {
            const { id } = req.params;
            const deleted = await ForumSectionElectric.destroy({ where: { id } });
...
module.exports = new ForumSectionElectricController();
