const { ScheduleElectric } = require('../../models/models'); // Проверьте путь к модели
const ApiError = require('../../error/ApiError');

class ScheduleElectricController {
    // Создать расписание
    async createSchedule(req, res, next) {
        try {
            ...
            const newSchedule = await ScheduleElectric.create({
                id_groupes,
                id_group,
                date,
            });
    ...
    async getSchedules(req, res, next) {
        try {
            const schedules = await ScheduleElectric.findAll();
    ...
    async getScheduleById(req, res, next) {
        try {
            const { id } = req.params;
            const schedule = await ScheduleElectric.findByPk(id);
    ...
    async deleteSchedule(req, res, next) {
        try {
            const { id } = req.params;
            const deleted = await ScheduleElectric.destroy({ where: { id } });
...
module.exports = new ScheduleElectricController();
