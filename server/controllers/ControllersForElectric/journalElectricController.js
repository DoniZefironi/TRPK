const { JournalElectric } = require('../../models/models'); // Проверьте путь к модели
const ApiError = require('../../error/ApiError');

class JournalElectricController {
    ...
    async createJournal(req, res, next) {
        try {
            ...
            const newJournal = await JournalElectric.create({
                id_group,
                grades, // JSONB поле
                id_classes,
                change_date,
    ...
    async getJournals(req, res, next) {
        try {
            const journals = await JournalElectric.findAll();
    ...
    async getJournalById(req, res, next) {
        try {
            const { id } = req.params;
            const journal = await JournalElectric.findByPk(id);
    ...
    async deleteJournal(req, res, next) {
        try {
            const { id } = req.params;
            const deleted = await JournalElectric.destroy({ where: { id } });
...
module.exports = new JournalElectricController();
